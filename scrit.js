...
function setSyncStatus(status, text) {
  ...
  if (retryBtn) {
    retryBtn.classList.toggle('show', status === 'offline');
  }
}
...
function renderTeamDashboard() {
  ...
  const avgEl = document.getElementById('teamAvgScore');
  avgEl.textContent = teamPassRate.toFixed(1) + '%';
  avgEl.style.color = teamPassRate >= 85 ? 'var(--accent-green)' : teamPassRate >= 80 ? 'var(--accent-yellow)' : 'var(--accent-red)';
  ...
  const sorted = members.sort((a, b) => (b[1].percentage || 0) - (a[1].percentage || 0));
  ...
  sorted.forEach(([name, member], idx) => {
    ...
    // Check absent status - try Gist first, fallback to localStorage
    let absentList = [];
    if (teamData[ABSENT_GIST_KEY]) {
      absentList = teamData[ABSENT_GIST_KEY];
    } else {
      const absentKey = 'callTracker_absent_' + teamId;
      const localAbsent = localStorage.getItem(absentKey);
      if (localAbsent) absentList = JSON.parse(localAbsent);
    }
    const isAbsent = absentList.some(n => n.toLowerCase() === name.toLowerCase());
...
function updateDailyGoal() {
  ...
  badge.classList.toggle('hidden', pct < 100);
}
...
const ABSENT_GIST_KEY = 'absent_members';
...
function joinTeam() {
  ...
  nameOverlay.style.transition = 'opacity 0.6s ease, transform 0.6s ease';
  nameOverlay.style.opacity = '0';
  nameOverlay.style.transform = 'scale(1.1)';
  setTimeout(() => {
...
async function initGistAndSync() {
  ...
  try {
    ...
    // Load absent status from Gist if available
    const absentKey = 'callTracker_absent_' + teamId;
    if (data && data[ABSENT_GIST_KEY]) {
      localStorage.setItem(absentKey, JSON.stringify(data[ABSENT_GIST_KEY]));
    }
    renderTeamDashboard();
    render();
    setSyncStatus('online', 'Live');
